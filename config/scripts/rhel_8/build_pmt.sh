...
MODULES_DIR=$(ls -td -- /usr/src/kernels/*/ | head -n 1)
(
cd $PRODUCT_DIR
...
(
cd $PRODUCT_DIR
$KWPREFIXDKMS make -f Makefile.dkms BUILD_VERSION="$BUILD_NUMBER" dkmsrpm-pkg RPMOPTS="--define \"_rpmdir ./rpm/\""
)
...
mkdir -p output/pmt
cp $PRODUCT_DIR/rpm/x86_64/*.rpm output/pmt/
